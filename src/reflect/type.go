...
func (t Type) NumMethod() int {
	if t.isBasic() {
		...
		namedTypeNum := t >> 6
		return int(*(*byte)(unsafe.Pointer(uintptr(unsafe.Pointer(&namedBasicNumMethodSidetable)) + uintptr(namedTypeNum))))
	} else {
		if !t.isNamed() {
			// Most named types cannot have methods. However, structs can
